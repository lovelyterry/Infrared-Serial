...
class Infrared:
    ...
    def parse(self, buffer):
        ...
        # 寻找上一帧数据的结尾
        frameIndex = self.cache.find(b'MSGPACK\0')
        if (frameIndex >= 0 and len(self.cache) - frameIndex > 16):
            length = struct.unpack(
                '<I', self.cache[frameIndex+8:frameIndex+12])[0]
            crcrec = struct.unpack(
                '<I', self.cache[frameIndex+12:frameIndex+16])[0]
            if (len(self.cache) < frameIndex + 16 + length):
                return None
            msgstr = self.cache[frameIndex + 16:frameIndex + 16 + length]
            crccal = crc32(msgstr)
            if (crccal != crcrec):
                self.cache = self.cache[frameIndex+16:]
            self.cache = self.cache[frameIndex+16+length:]
            try:
                msg = msgpack.unpackb(msgstr, raw=True)
                if (msg[b'F'] == b"update" and msg[b'P'] == b'C'):
                    width = msg[b'M'][b'width']
                    height = msg[b'M'][b'height']
                    data = msg[b'M'][b'data']
                    tempMatrix = np.zeros([width, height], dtype=float)
                    for i in range(int(len(data) / 2)):
                        temp = struct.unpack("H", data[i*2:i*2+2])[0]
                        y = int(i / width)
                        x = int(i % width)
                        # 解析原始数据为摄氏度，原始数据为开尔文温度，量纲为0.1，这里转换为摄氏度：除以10得到开尔文温度，再减去273.15(绝对零度)
                        tempMatrix[y][x] = (float)(temp) / 10.0 - 273.15
                    self.printTemp(tempMatrix)
                    return tempMatrix
                return None
