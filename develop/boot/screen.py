    ...
    def _compress(self, raw_data: bytes) -> bytearray:
        ...
        if self.compress == CompressMethod.RLE:
            ...
            if self.raw_data_len % self.blk_size:
                pad = b'\x00' * (self.blk_size -
                                 self.raw_data_len % self.blk_size)
...
def main():
    ...
    bootimg = "./bootimg.png"
    if path.isfile(bootimg):
        files = [bootimg]
    elif path.isdir(bootimg):
        files = list(Path(bootimg).rglob("*.[pP][nN][gG]"))
    else:
        raise BaseException(f"invalid input: {bootimg}")
...
def upload():
    ...
    file_path = "./bootimg.bin"
    try:
        with open(file_path, "rb") as file:
            files = {"file": ("bootimg.bin", file,
